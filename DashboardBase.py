...
df["calf_strength_left"] = df["calf_force_left"] / df["body_mass"]
df["calf_strength_right"] = df["calf_force_right"] / df["body_mass"]
    ...
    html.Div([
        ...
        html.Div([
            ...
            html.Div([
                ...
                html.Div([
                    html.H3("Jump Height Asymmetry", style={
                        "textAlign": "center",
                        "color": COLORS['secondary'],
                        "marginBottom": "5px",
                        "fontSize": "14px"
...
def update_dashboard(selected_athlete, selected_date):
    ...
    calf_fig.add_trace(go.Scatter(
        ...
        hovertemplate='%{y:.2f} N.kg⁻¹<extra>Left</extra>'
    ))
    calf_fig.add_trace(go.Scatter(
        x=athlete_df["date"],
        ...
        hovertemplate='%{y:.2f} N.kg⁻¹<extra>Right</extra>'
    ))
    
    # Add target line (keeping this as it's an important reference)
    calf_fig.add_hline(
        y=2.0,
        line_dash="dash",
        line_color=COLORS['secondary'],
        line_width=2,
        annotation_text="Target: 2.0 N.kg⁻¹",
        annotation_position="right",
        annotation_font_size=12,
        annotation_font_color=COLORS['secondary']
    )
    
    # Add 70% threshold line (keeping this as it's an important reference)
    calf_fig.add_hline(
        y=1.4,  # 70% of 2.0
        line_dash="dot",
        line_color=COLORS['secondary'],
        line_width=1,
        annotation_text="70% Target: 1.4 N.kg⁻¹",
        annotation_position="right",
        annotation_font_size=10,
        annotation_font_color=COLORS['secondary']
    )
    ...
    return (selected_athlete,
            f"Age: {age:.1f} years | Injury Date: {latest['injury_date'].strftime('%d/%m/%Y')} | Days Post-Injury: {injury_days}",
